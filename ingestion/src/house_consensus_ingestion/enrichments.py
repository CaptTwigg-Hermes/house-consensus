...
def _image_urls(record: Mapping[str, Any]) -> list[str]:
    ...
    return [url for _, url in sorted(result, reverse=True)[:10]]
    ...
    def enrich(self, records: list[dict[str, Any]]) -> dict[str, int]:
        ...
        for record in records:
            ...
            if result is None:
                ...
                if result.get("vision_run_status") != "ok":
...
class SoldHistoryEnricher:
    ...
    def enrich(self, records: list[dict[str, Any]]) -> dict[str, int]:
        ...
        for record in records:
            ...
            if value is None:
                sales = [dict(item) for item in self._fetch(address_id)]
                value = {"sales": sales}
                self._cache.put("sold-history-v1", address_id, value); fresh += 1
            else:
                sales = value.get("sales") or []; hits += 1
            record["sold_history"] = [{
                "date": sale.get("date"), "amount_dkk": sale.get("amount_dkk", sale.get("amount")),
                "per_m2": sale.get("per_m2", sale.get("perAreaPrice")), "type": sale.get("type"),
            } for sale in sales if isinstance(sale, Mapping)]
            record["sold_history_status"] = "ok"
        return {"enriched": len(records), "fresh": fresh, "cache_hits": hits}
        ...
        with self._connection_factory() as connection:
            for record in records:
                ...
                with connection.cursor() as cursor:
                    cursor.execute("""
                        SELECT catalog.noise_source, catalog.indicator,
                               CASE WHEN area.area_id IS NULL THEN 'no_contour' ELSE 'covered' END,
                               area.db_value, area.db_band, catalog.source_key
                        FROM ingestion_noise_source AS catalog
                        LEFT JOIN LATERAL (
                            SELECT area_id, db_value, db_band
                            FROM ingestion_noise_area
                            WHERE source_id = catalog.source_id
                              AND ST_Intersects(geometry, ST_SetSRID(ST_MakePoint(%s, %s), 4326))
                            ORDER BY db_value DESC NULLS LAST LIMIT 1
                        ) AS area ON true
                        WHERE catalog.available
                        ORDER BY catalog.noise_source, catalog.indicator
                    """, (float(lon), float(lat)))
                    rows = cursor.fetchall()
                sources: dict[str, dict[str, Any]] = {}
                for source, indicator, status, db_value, db_band, source_key in rows:
                    sources.setdefault(str(source), {})[str(indicator)] = {
                        "status": str(status), "db_value": db_value, "db_band": db_band, "source_key": source_key,
                    }
                    covered += status == "covered"
                road = sources.get("ROAD", {}).get("Lden", {})
                db = road.get("db_value")
                if road.get("status") == "no_contour": aggregate = "quiet"
                elif _finite(db) and float(db) >= 55: aggregate = "noisy"
                elif _finite(db): aggregate = "quiet"
                else: aggregate = "unknown"
                record.update(noise_status=aggregate, noise_sources=sources)
        return {"enriched": len(records), "covered_observations": covered}
