...
public sealed record ListingDto(Guid Id, string ExternalId, string Address, string? City, decimal? Price, double FamilyFitScore, ListingState State, bool AiAssessed, double? AiConfidence, string? AiEvidence, string? ModelVersion, string? RuleVersion, string? SourceUrl, bool Consensus, IReadOnlyCollection<VoteDto> Votes);
