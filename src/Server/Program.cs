...
listings.MapPut("/{id:guid}/asbestos-roof-correction", async (Guid id, SetAsbestosRoofCorrection request, ClaimsPrincipal user, AppDbContext db, CancellationToken ct) =>
{
    if (!request.Confirmed) return Results.BadRequest(new { error = "Confirmation is required." });
    await using var transaction = await db.Database.BeginTransactionAsync(ct);
    await LockListingMutation(id, db, ct);
    if (!await CanAccessListing(id, user, db, ct)) return Results.NotFound();
    var listing = await db.Listings.SingleAsync(x => x.Id == id, ct);
    listing.SetAsbestosRoofCorrection(request.Status);
    await db.SaveChangesAsync(ct);
    await transaction.CommitAsync(ct);
...
static AsbestosRoofStatus? ParseAsbestosRoofStatus(string? value) => value switch
{
    "likely" => AsbestosRoofStatus.Likely,
    "possible" => AsbestosRoofStatus.Possible,
    "no_indication" => AsbestosRoofStatus.NoIndication,
    "unknown" => AsbestosRoofStatus.Unknown,
    _ => null,
};
