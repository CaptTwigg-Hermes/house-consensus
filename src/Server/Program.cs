...
listings.MapGet("/my-votes", async (ClaimsPrincipal user, AppDbContext db, CancellationToken ct) =>
{
    ...
    var votes = await db.Votes.Where(x => x.MemberId == id).OrderByDescending(x => x.CreatedAt).ThenByDescending(x => x.Id).ToListAsync(ct);
...
listings.MapPost("/{id:guid}/votes", async (Guid id, CastVote request, ClaimsPrincipal user, AppDbContext db, IHubContext<ConsensusHub> hub, TimeProvider clock, CancellationToken ct) => { if (request.Choice == VoteChoice.NotVoted) return Results.BadRequest(new { error = "Clearing votes is not supported." }); if (!await db.Listings.AnyAsync(x => x.Id == id && (x.State == ListingState.Active || x.State == ListingState.Restored), ct)) return Results.NotFound(); var vote = new Vote(id, user.MemberId(), request.Choice, request.Tags ?? [], request.Note, clock.GetUtcNow()); db.Votes.Add(vote); await db.SaveChangesAsync(ct); var consensus = await HasConsensus(id, db, ct); var dto = (await VoteDtos([vote], db, ct))[0]; await hub.Clients.Group($"listing:{id}").SendAsync("VoteChanged", dto, consensus, ct); await hub.Clients.All.SendAsync("ConsensusChanged", id, consensus, ct); return Results.Ok(new { vote = dto, consensus }); });
...
static VoteDto ToVoteDto(Vote v, string memberInitials = "", string memberColor = "") => new(v.Id, v.ListingId, v.MemberId, v.Choice, v.Tags, v.CreatedAt, v.Note, memberInitials, memberColor);
