...
listings.MapGet("/queue", async (ClaimsPrincipal user, AppDbContext db, CancellationToken ct) => { var all = await ListingDtos(db.Listings.Where(x => x.State == ListingState.Active || x.State == ListingState.Restored).OrderByDescending(x => x.IsManuallyAdded).ThenByDescending(x => x.ManuallyAddedAt), db, user, ct); return all.Where(x => !x.Votes.Any(v => v.MemberId == user.MemberId() && v.Choice != VoteChoice.NotVoted)).OrderByDescending(x => x.IsManuallyAdded).ThenByDescending(x => x.ManuallyAddedAt).ThenByDescending(x => x.TrustedFamilyFitScore.HasValue).ThenByDescending(x => x.TrustedFamilyFitScore); });
