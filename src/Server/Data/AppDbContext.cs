...
public sealed class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<AiRuleApplication>(e => { e.ToTable("ai_rule_applications"); e.HasKey(x => x.Id); e.Property(x => x.Id).ValueGeneratedOnAdd(); e.HasIndex(x => new { x.ProposalId, x.ListingId }).IsUnique(); e.HasOne<AiRuleProposal>().WithMany().HasForeignKey(x => x.ProposalId).OnDelete(DeleteBehavior.Restrict); e.HasOne<Listing>().WithMany().HasForeignKey(x => x.ListingId).OnDelete(DeleteBehavior.Restrict); });
