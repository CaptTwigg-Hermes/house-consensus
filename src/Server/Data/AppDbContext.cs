...
public sealed class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
{
    ...
    public DbSet<VoteRating> VoteRatings => Set<VoteRating>();
    ...
    public override int SaveChanges(bool acceptAllChangesOnSuccess) { InitializeVotingIdentities(); return base.SaveChanges(acceptAllChangesOnSuccess); }
    public override Task<int> SaveChangesAsync(bool acceptAllChangesOnSuccess, CancellationToken cancellationToken = default) { InitializeVotingIdentities(); return base.SaveChangesAsync(acceptAllChangesOnSuccess, cancellationToken); }
    private void InitializeVotingIdentities()
    {
        foreach (var entry in ChangeTracker.Entries<Member>().Where(x => x.State == EntityState.Added && x.Entity.VotingIdentityId == Guid.Empty)) entry.Entity.VotingIdentityId = entry.Entity.Id;
    }
    protected override void OnModelCreating(ModelBuilder b)
    {
        b.HasPostgresEnum<MemberRole>(); b.HasPostgresEnum<VoteChoice>(); b.HasPostgresEnum<ListingState>(); b.HasPostgresEnum<ReasonTag>(); b.HasPostgresEnum<VoteCategory>(); b.HasPostgresEnum<CategoryRating>(); b.HasPostgresEnum<OverrideAction>();
        b.Entity<Member>(e => { e.ToTable("members"); e.HasKey(x => x.Id); e.HasIndex(x => x.Email).IsUnique(); e.HasIndex(x => x.VotingIdentityId); e.Property(x => x.Email).HasMaxLength(320); e.Property(x => x.DisplayName).HasMaxLength(40); e.Property(x => x.AvatarColor).HasMaxLength(7); e.Property(x => x.Language).HasMaxLength(2); });
        b.Entity<Listing>(e => { e.ToTable("listings"); e.HasKey(x => x.Id); e.HasIndex(x => x.ExternalId).IsUnique(); e.HasIndex(x => x.CanonicalUrl).IsUnique().HasFilter("\"CanonicalUrl\" IS NOT NULL"); e.HasIndex(x => x.NormalizedAddress).IsUnique().HasFilter("\"NormalizedAddress\" IS NOT NULL"); e.HasIndex(x => new { x.State, x.FamilyFitScore }); e.Property(x => x.CanonicalUrl).HasMaxLength(2048); e.Property(x => x.NormalizedAddress).HasMaxLength(500); e.HasOne<Member>().WithMany().HasForeignKey(x => x.ManuallyAddedById).OnDelete(DeleteBehavior.Restrict); e.Property(x => x.Price).HasPrecision(14, 2); e.Property(x => x.BuildableStatus).HasMaxLength(64); e.Property(x => x.Condition).HasMaxLength(64); e.Property(x => x.GardenOrientation).HasMaxLength(64); e.Property(x => x.MultigenFit).HasMaxLength(64); e.Property(x => x.PostalCode).HasMaxLength(16); e.Property(x => x.FamilyUnits).HasMaxLength(64); e.Property(x => x.ScoreRuleVersion).HasMaxLength(100); e.Property(x => x.ScoreNotesJson).HasColumnType("text"); e.Property(x => x.RoadNoiseStatus).HasMaxLength(20); e.Property(x => x.RoadNoiseLnightStatus).HasMaxLength(20); e.Property(x => x.RailNoiseStatus).HasMaxLength(20); e.Property(x => x.RailNoiseLnightStatus).HasMaxLength(20); e.Property(x => x.AirNoiseStatus).HasMaxLength(20); e.Property(x => x.AirNoiseLnightStatus).HasMaxLength(20); e.HasMany(x => x.Overrides).WithOne().HasForeignKey(x => x.ListingId).OnDelete(DeleteBehavior.Restrict); });
        ...
        b.Entity<Vote>(e => { e.ToTable("votes"); e.HasKey(x => x.Id); e.Property(x => x.Id).ValueGeneratedOnAdd(); e.Property(x => x.Tags).HasColumnType("reason_tag[]"); e.Property(x => x.Note).HasMaxLength(2000); e.ToTable(t => t.HasCheckConstraint("CK_votes_OverallScore", "\"OverallScore\" IS NULL OR (\"OverallScore\" >= 1 AND \"OverallScore\" <= 5)")); e.HasMany(x => x.NoteRevisions).WithOne().HasForeignKey(x => x.VoteId).OnDelete(DeleteBehavior.Restrict); e.HasMany(x => x.Ratings).WithOne().HasForeignKey(x => x.VoteId).OnDelete(DeleteBehavior.Restrict); e.HasIndex(x => new { x.ListingId, x.MemberId, x.CreatedAt }); e.HasOne<Listing>().WithMany().HasForeignKey(x => x.ListingId).OnDelete(DeleteBehavior.Restrict); e.HasOne<Member>().WithMany().HasForeignKey(x => x.MemberId).OnDelete(DeleteBehavior.Restrict); });
        b.Entity<VoteRating>(e => { e.ToTable("vote_ratings"); e.HasKey(x => x.Id); e.Property(x => x.Id).ValueGeneratedOnAdd(); e.HasIndex(x => new { x.VoteId, x.Category }).IsUnique(); });
