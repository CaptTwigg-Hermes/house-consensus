...
fail() {
    code=$1
    shift
    printf 'native manual scoring: %s\n' "$*" >&2
    exit "$code"
}
...
for argument do
    case "$argument" in
        --d*)
            fail 64 "database credentials must come from the environment or HOUSE_CONSENSUS_ENV_FILE"
            ;;
    esac
done
...
[ -d "$root/manual_scoring" ] || fail 72 "manual_scoring project is missing under HOUSE_CONSENSUS_ROOT"
...
lock_file=$lock_dir/native-manual-scoring.lock
touch "$lock_file" || fail 73 "cannot open manual-scoring lock"
exec 9>"$lock_file"
flock -n 9 || fail 75 "native manual scoring is already running"

cd -- "$root"
printf 'native manual scoring: start root=%s\n' "$root" >&2
set +e
uv run --project manual_scoring house-consensus-manual-scorer "$@"
status=$?
set -e
printf 'native manual scoring: finish exit=%s\n' "$status" >&2
exit "$status"
